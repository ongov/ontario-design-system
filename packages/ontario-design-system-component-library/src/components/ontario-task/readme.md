...
| `taskStatus`     | `task-status`     | Defines the status of the task, with default set to 'NotStarted'. Accepts values from `TaskStatuses` enum: `NotStarted`, `InProgress`, `Completed`.                                                                 | `"cannotStartYet" \| "completed" \| "error" \| "inProgress" \| "notStarted" \| "optional"` | `undefined` |
